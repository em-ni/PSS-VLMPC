...
typedef RT_MODEL_send_pressure_udp_T RT_MODEL;

/*====================*
 * External functions *
 *====================*/
EXTERN RT_MODEL * send_pressure_udp(void);
  ...
  _do_assertion(const char * expression, const char * file_name, int line_number)
{
  ...
  rtmSetErrorStatus(send_pressure_udp_M, GBLbuf.message);
}
  ...
  main(int argc, char * argv[])
{
  ...
  if (parseError) {
    (void) fprintf(stderr,
                   "\nUsage: send_pressure_udp -option1 val1 -option2 val2 -option3 ...\n\n");
    (void) fprintf(stderr,
