...
import joblib
from train_sim import StatePredictor, load_and_prepare_data, TrainingConfig
...
if __name__ == "__main__":
    ...
    print("--- Loading data, model, and scalers for Torque-Based Model ---")
    try:
        # CRUCIAL: Use the *exact same* data preparation function as in training.
        # This is the new function that handles the torque data and single trajectory.
        ...
        print("Please run the 'train_sim.py' script first to generate the model and scaler files.")
        sys.exit()

    # --- 2. Recreate the exact same test split ---
    # Using the same random_state from TrainingConfig ensures we get the identical test set.
    ...
    # The input and output dimensions are correctly inferred from the test data shape.
    # This automatically adapts to the new [torques, state, dt] input format.
    ...
    # --- 6. Plot the results ---
    print("\n--- Generating plots ---")
    # The plot function is generic and works perfectly for the new model's output.
    # It saves the plot to the path specified in the new TrainingConfig.
