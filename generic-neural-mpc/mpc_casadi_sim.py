...
class MPCConfig:
    ...
    r_diag = 0.0
    R_diag = [r_diag, r_diag, r_diag, r_diag]  # 4 torque inputs
    r_rate_diag = 10.0
    R_rate_diag = [r_rate_diag, r_rate_diag, r_rate_diag, r_rate_diag]  # 4 torque inputs
    max_torque = 9e-2
...
def run_mpc_simulation():
    ...
    try:
        df = pd.read_csv(MPCConfig.SIM_DATASET_PATH)
        df.columns = df.columns.str.strip()
        model = StatePredictor(input_dim=11, output_dim=6)  # 4 torques + 6 states + 1 dt = 11 inputs
        model.load_state_dict(torch.load(MPCConfig.MODEL_PATH))
        # --- JIT Compile the model for extra speed ---
        dummy_input = torch.randn(1, 11) # 1 sample, 11 features
    ...
    for k in range(MPCConfig.N):
        ...
        cost += U[:, k].T @ R @ U[:, k]
        if k == 0: 
            cost += (U[:, k] - u_prev).T @ R_rate @ (U[:, k] - u_prev)
        else: 
    ...
    state_cols = ['tip_position_x', 'tip_position_y', 'tip_position_z', 
    ...
    for i in range(n_steps):
        ...
        for k in range(MPCConfig.N):
            J_k = J_batch[k]
            B_k = J_k[:, :n_controls]  # First 4 columns for torque inputs
            A_k = J_k[:, n_controls:n_controls+n_states]  # Next 6 columns for states
    ...
    plt.tight_layout()
    plot_path = 'results/mpc_casadi_sim.png'
...
if __name__ == "__main__":
    ...
    model_g = StatePredictor(input_dim=11, output_dim=6)
